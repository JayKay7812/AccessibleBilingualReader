...
function s(){
  i.attr("src","http://apii.dict.cn/apis/dict3.php?skin=default&q="+encodeURIComponent(I)+"#"+(ak?1:0))
}
function b(a){
  if(!a){
    a=""
  ...
  function at(a,aD){
    al.cookie(a,aD,{expires:3650,domain:am.domain})
  }
  function af(a){
    var aD=al("#"+a);
  ...
  function N(a,aH){
    return;
    var aF=aH,aG=aF("#"+a),aE=aF("[slot]",aG);

    if(aE.length){
      var aD="http://list.dict.cn/list/slots.php?callback=?&id=";
      ...
      aF.getJSON(aD,function(aJ){
        if(aJ){
          ...
          aF.each(aJ,function(aK,aL){
            ...
            switch(aL.type){
              case"text":aL.dom=aF('<a title="'+aL.text+'" target="_blank" href="'+aL.href+'">'+aL.text+"</a>");
              break;
              case"image":aL.dom=aF('<a target="_blank" href="'+aL.href+'"><img alt="'+aL.title+'" src="'+aL.src+'"></a>');
              break;
              case"slide":if(aL.slot){
                !function(aM){
